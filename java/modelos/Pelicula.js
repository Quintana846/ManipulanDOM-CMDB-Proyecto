 ...
 * Incluye lógica de cálculo de media (Unidad 3).
 */
export class Pelicula {
    ...
    set titulo(valor) { 
        if(valor && valor.length <= 100) this._titulo = valor; 
    }

    get fechaEstreno() { return this._fechaEstreno; }
    set fechaEstreno(valor) { this._fechaEstreno = valor; }

    get popularidad() { return this._popularidad; }
    set popularidad(valor) {
        if(valor >= 0 && valor <= 100) this._popularidad = valor;
    }

    get generos() { return this._generos; }
    set generos(valor) { this._generos = valor; }

    /**
     * Calcula la media de las puntuaciones.
     * Requisito: Se calcula en tiempo real al consultar[cite: 35].
     * @returns {string} - Media con 1 decimal o "0.0".
     */
    get puntuacionMedia() {
        if (this._puntuaciones.length === 0) return "0.0";
        // Uso de reduce (Unidad 3.2.7)
        const suma = this._puntuaciones.reduce((acc, val) => acc + val, 0);
        return (suma / this._puntuaciones.length).toFixed(1);
    }
     ...
     * @returns {number}
     */
    get numeroVotos() { return this._puntuaciones.length; }
